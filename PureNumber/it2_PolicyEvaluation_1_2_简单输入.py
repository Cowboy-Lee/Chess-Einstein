...
# 电脑学习爱恩斯坦棋的规则。
#
#   —— 2层结构输入，4层网络（2Conv+2FC），只学习可走子规则。
#
#
...
CHECKPOINT_PATH = "Checkpoints_it2_pol1_normal/"
SUMMARY_PATH = "Summary/it2_Policy1_SimpleInput/Part3Results/"
...
def createNetwork():
    ...
    W_fc1 = weight_variable([25*64, 128], "policy_L3_fc_W")
    b_fc1 = bias_variable([128], "policy_L3_fc_B")

    W_fc2 = weight_variable([128, ACTIONS], "policy_L4_fc_W")
    b_fc2 = bias_variable([ACTIONS], "policy_L4_fc_B")
    ...
    h_conv2_flat = tf.reshape(h_conv2, [-1, 25*64])
    ...
    # readout layer
    readout = tf.matmul(h_fc1_drop, W_fc2, name="outLayer") + b_fc2

    return s, readout

def trainNetwork(s, readout, sess):
    # define the cost function
    available = tf.placeholder("float", [None, ACTIONS])
    ...
    tf.summary.scalar("loss_it2_pol1_simpleInput", loss)
    ...
    while "Einstein_Policy" != "Einstein":
        ...
        if t % 100000 == 0:
            saver.save(sess, CHECKPOINT_PATH + GAME , global_step = t)
            print("graph saved.")
