...
# 电脑学习爱恩斯坦棋的规则。
#
#   —— 单层结构输入，5层网络（3Conv+2FC），只学习棋子是否存在。
#
#
...
IsTraining = False
CHECKPOINT_PATH = "Checkpoints_ChessExistance/"
SUMMARY_PATH = "Summary/it3_ChessLogic1_Existance/"
...
def trainNetwork(s, readout, sess):
    ...
    if not IsTraining:
        correct_prediction = tf.equal(tf.argmax(available, axis=1), tf.argmax(readout, axis=1))
    ...
    while "Einstein_ChessLogic" != "Einstein":
        ...
        if not IsTraining:
            prediction = sess.run(correct_prediction, feed_dict={s:[s_t], available:[av]})
            sumPredict += prediction
            if t%10==0:
                print("t = %d\t| acc = %.2f" % (t, sumPredict/(t+1)))
            if t>=3000:
                exit()
        else:
        ...
        if t%1000==0:
            ...
            if not IsTraining and t%10000==0:
